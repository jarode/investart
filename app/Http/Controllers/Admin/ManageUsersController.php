...
class ManageUsersController extends Controller
{
    ...
    public function ckycUnverifiedUsers()
    {
        $pageTitle = 'Case Creator KYC Unverified Users';
        $users = $this->userData('ckycUnverified');
        return view('admin.users.list', compact('pageTitle', 'users'));
    }

    public function ckycPendingUsers()
    {
        $pageTitle = 'Case Creator KYC Unverified Users';
        $users = $this->userData('ckycPending');
        return view('admin.users.list', compact('pageTitle', 'users'));
    }
    ...
    public function detail($id)
    {
        ...
        $totalCase = InvestmentCase::active()->where('user_id',$user->id)->count();
        $totalInvest = Invest::where('user_id',$user->id)->sum('invest_amount');
        $totalProfit = ProfitReturn::where('to_user',$user->id)->active()->sum('profit_amount');
        $invest = Invest::where('user_id',$user->id)->count();
        $countries = json_decode(file_get_contents(resource_path('views/partials/country.json')));
        return view('admin.users.detail', compact('pageTitle','invest','totalProfit','totalInvest','totalCase','user','totalDeposit','totalWithdrawals','totalTransaction','countries'));
    }
    ...
    public function ckycDetails($id)
    {
        $pageTitle = 'case KYC Details';
        $user = User::findOrFail($id);
        return view('admin.users.ckyc_detail', compact('pageTitle','user'));
    }
    ...
    public function ckycApprove($id)
    {
        $user = User::findOrFail($id);
        $user->ckv = 1;
        $user->save();
        ...
        $notify[] = ['success','Case KYC approved successfully'];
        return to_route('admin.users.ckyc.pending')->withNotify($notify);
    }
    ...
    public function ckycReject($id)
    {
        $user = User::findOrFail($id);
        foreach ($user->ckv_data as $kycData) {
            if ($kycData->type == 'file') {
                fileManager()->removeFile(getFilePath('verify').'/'.$kycData->value);
            }
        }
        ...
        $notify[] = ['success','CaSE KYC rejected successfully'];
        return to_route('admin.users.kyc.pending')->withNotify($notify);
    }
